...
export function PropertyButton({ property, label, icon: Icon }: Props) {
  ...
  const hasActiveWidget = useInteractionStore((s) => s.floatingWidgets.some((w) => w.property === property));
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (widgetIdRef.current) {
        updateWidgetPosition(widgetIdRef.current, clientToWidgetLayerPosition(e.clientX, e.clientY), { x: 0, y: 0 });
      }
    },
  ...
  return (
    <div className={cn("flex w-11 flex-col items-center gap-1", hasActiveWidget && "pointer-events-none opacity-30")}>
      <div className={cn("flex h-9 w-9 cursor-grab items-center justify-center rounded-md border border-border p-2 shadow-sm transition-colors active:cursor-grabbing", isDragging ? "bg-accent" : "hover:bg-accent/50")} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
