...
export function Header() {
  ...
  return (
    <>
      <header className={cn("flex h-12 shrink-0 items-center justify-between border-b border-sidebar-border bg-sidebar pl-3 pr-2")}>
        {/* Left: logo + nav */}
          ...
          <Menubar className="h-auto gap-0 border-none bg-transparent p-0">
            <MenubarMenu>
              ...
              <MenubarContent>
                ...
                <MenubarItem className="whitespace-nowrap" onClick={() => setProjectBrowserOpen(true)}>
                  Open…
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={handleSave}>
                  Save{isDirty && !savedIndicator ? "" : ""}
                  {savedIndicator && <Check className="ml-auto h-3.5 w-3.5 text-muted-foreground" />}
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={() => setSaveAsOpen(true)}>
                  Save as…
                </MenubarItem>
                <MenubarSeparator />
                <MenubarItem className="whitespace-nowrap" onClick={() => setImportOpen(true)}>
                  Import…
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={() => setExportOpen(true)}>
                  Export…
                </MenubarItem>
                <MenubarSeparator />
                <MenubarItem className="whitespace-nowrap" onClick={() => setCanvasSettingsOpen(true)}>
                  Canvas settings…
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={() => setExportVideoOpen(true)}>
                  Export video…
                </MenubarItem>
              </MenubarContent>
            </MenubarMenu>
            ...
            <MenubarMenu>
              ...
              <MenubarContent>
                <MenubarItem disabled={!canUndo} onClick={() => useAnimationStore.temporal.getState().undo()}>
                  Undo
                </MenubarItem>
                <MenubarItem disabled={!canRedo} onClick={() => useAnimationStore.temporal.getState().redo()}>
                  Redo
                </MenubarItem>
                <MenubarSeparator />
                <MenubarItem className="whitespace-nowrap" onClick={() => setHeldLayers(getFlatRenderIds(doc))}>
                  Select all
                </MenubarItem>
                <MenubarSeparator />
                <MenubarItem className="whitespace-nowrap" disabled={heldLayerIds.length === 0} onClick={() => setLayersHidden(heldLayerIds, true)}>
                  ...
                  onClick={() =>
                    setLayersHidden(
                      Object.values(doc.layers)
                        .filter((l) => l.type === "layer" && l.hidden)
                        .map((l) => l.id),
                      false,
                    )
                  }
            ...
            <MenubarMenu>
              ...
              <MenubarContent>
                <MenubarItem className="whitespace-nowrap" onClick={() => zoomByFactor(CANVAS_ZOOM_STEP)}>
                  Zoom in
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={() => zoomByFactor(1 / CANVAS_ZOOM_STEP)}>
                  Zoom out
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={() => setZoomPreset(1)}>
                  Zoom to 100%
                </MenubarItem>
                <MenubarItem className="whitespace-nowrap" onClick={fit}>
                  Zoom to fit
                </MenubarItem>
              </MenubarContent>
            </MenubarMenu>
            ...
            <MenubarMenu>
              ...
              <MenubarContent>
                <MenubarItem className="whitespace-nowrap" onClick={() => setHelpOpen(true)}>
                  Help...
                </MenubarItem>
                {isTouchDevice && !isInstalledPWA && (
                  <MenubarItem className="whitespace-nowrap" onClick={() => setAddToHomeOpen(true)}>
                    Add shortcut to home screen
                  </MenubarItem>
                )}
                <MenubarItem className="whitespace-nowrap" onClick={() => setAboutOpen(true)}>
                  About Muy
                </MenubarItem>
              </MenubarContent>
            </MenubarMenu>
      ...
      <SaveAsDialog open={saveAsOpen} onOpenChange={setSaveAsOpen} defaultName={currentProjectName ?? "Untitled"} onConfirm={handleSaveAs} />
