...
import { MousePointer2, Pencil, Eraser, Crosshair, Film, Type, Search, Settings2, Gauge, Lightbulb, MoveHorizontal, MoveVertical, RotateCw, Scaling, Blend, Eye } from "lucide-react";
        ...
        items: [
          {
            ...
            detail: (
              <>
                Import image layers via the + button in the Layers panel, or draw with the <Pencil className="inline h-3 w-3 mx-0.5 -mt-0.5" /> Pencil or <Type className="inline h-3 w-3 mx-0.5 -mt-0.5" /> Text tool. On desktop you can also drag image files directly onto the canvas.
              </>
            ),
          },
          {
          ...
          },
          {
            ...
            detail: (
              <>
                Pick the <Film className="inline h-3 w-3 mx-0.5 -mt-0.5" /> Animate tool. Choose a property. Tap Play, then drag the widget to record movement in real time.
              </>
            ),
          },
          {
        ...
        items: [
          {
            ...
            detail: (
              <>
                Tap a layer to select it; tap again to deselect. With the <MousePointer2 className="inline h-3 w-3 mx-0.5 -mt-0.5" /> Select tool active, dragging the canvas moves all selected layers at the current frame.
              </>
            ),
          },
        ],
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        heading: "Properties",
        items: [
          { icon: MoveHorizontal, label: "Move X", detail: "Shifts the layer horizontally." },
          { icon: MoveVertical, label: "Move Y", detail: "Shifts the layer vertically." },
          { icon: RotateCw, label: "Rotate", detail: "Rotates the layer. The center of rotation is the pivot point." },
          { icon: Scaling, label: "Scale", detail: "Resizes the layer. Scaling is relative to the pivot point." },
          { icon: Blend, label: "Alpha", detail: "Controls the opacity of the layer, from fully transparent to fully opaque." },
          { icon: Eye, label: "Reveal", detail: "Progressively reveals or hides vector strokes and text. Not available for image layers." },
        ],
      },
    ],
  },
        ...
        items: [
          {
            ...
            detail: (
              <>
                Each layer has a sensitivity %. Tap the <Settings2 className="inline h-3 w-3 mx-0.5 -mt-0.5" /> icon in the Layers panel header to reveal the sensitivity scrubbers. Lower values give subtler, smoother motion; higher values give larger, faster movements.
              </>
            ),
          },
          {
          ...
          },
          {
            ...
            detail: "Flick a property widget quickly to dismiss it.",
          },
          {
            icon: Lightbulb,
            label: "Undo a take",
...
function matchesQuery(article: Article, q: string): boolean {
  ...
  const text = [article.title, ...article.sections.flatMap((s) => [s.heading ?? "", s.body ?? "", ...(s.items ?? []).flatMap((it) => [it.label, it.detail ?? ""])])].join(" ").toLowerCase();
...
export function HelpDialog({ open, onOpenChange }: Props) {
  ...
  const filtered = useMemo(() => visibleArticles.filter((a) => matchesQuery(a, query)), [query, visibleArticles]);
        ...
        <aside className="flex w-48 shrink-0 flex-col border-r border-border">
          <div className="p-3 border-b border-border">
            ...
            <div className="relative">
              ...
              <Input placeholder="Search…" value={query} onChange={(e) => setQuery(e.target.value)} className="h-8 pl-8 text-sm" />
            </div>
          </div>
          <nav className="flex-1 overflow-y-auto p-1.5 space-y-0.5">
            {(query ? filtered : visibleArticles).map((a) => (
              <button
                ...
                className={cn("w-full rounded-md px-3 py-1.5 text-left text-sm transition-colors", !query && activeId === a.id ? "bg-accent text-foreground font-medium" : "text-muted-foreground hover:bg-accent/50 hover:text-foreground")}
              >
                {a.title}
              </button>
            ))}
            {query && filtered.length === 0 && <p className="px-3 py-2 text-sm text-muted-foreground">No results</p>}
          </nav>
        </aside>
            ...
            <>
              ...
              <div className="space-y-5">
                {displayed.sections.map((section, si) => (
                  <div key={si}>
                    {section.heading && <p className="mb-2 text-sm font-semibold text-muted-foreground">{section.heading}</p>}
                        ...
                        {section.items.map((item, ii) => {
                          ...
                          return (
                            <li key={ii} className="flex items-start gap-2.5">
                              ...
                              {!Icon && <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-accent text-[10px] font-semibold text-foreground">{ii + 1}</span>}
                              <div>
                                <p className="text-sm font-semibold leading-snug">{item.label}</p>
                                {item.detail && <p className="text-sm text-muted-foreground leading-snug mt-0.5">{item.detail}</p>}
                              </div>
                            </li>
                          );
                        })}
