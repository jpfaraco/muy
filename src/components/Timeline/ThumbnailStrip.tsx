...
export function ThumbnailStrip() {
  ...
  return (
    <div
      className="relative h-9 w-full cursor-grab overflow-hidden rounded-lg bg-background active:cursor-grabbing"
    ...
    >
      {/* Scrolling filmstrip */}
      <div
        className="absolute top-0.5 flex"
        style={{ left: stripLeft, gap: THUMB_GAP }}
      >
        {Array.from({ length: doc.frameCount }, (_, i) => {
          ...
          return (
            <div
              ...
              className={cn(
                'relative shrink-0',
                i === currentFrame ? 'bg-muted-foreground/60' : 'bg-secondary',
                state === 'no-selection' && 'opacity-50',
              )}
              style={{ width: THUMB_W, height: THUMB_H, borderRadius: 1 }}
            >
              {state === 'keyframe' && (
                <div
                  className="absolute left-1/2 -translate-x-1/2 rounded-full bg-foreground"
                  style={{ width: 4, height: 4, top: 4 }}
                />
              )}
              {state === 'partial' && (
                <div
                  className="absolute left-1/2 -translate-x-1/2 rounded-full border border-foreground"
                  style={{ width: 4, height: 4, top: 4 }}
                />
              )}
            </div>
      ...
      <div className="pointer-events-none absolute inset-y-0 left-1/2 w-0.5 -translate-x-px bg-red-500" />
